...
\begin{lstlisting}[language=Python, caption={Send RTSP Request - SETUP}]
...
\begin{lstlisting}[language=Python, caption={Send RTSP Request - PLAY/PAUSE/TEARDOWN}]
...
\textbf{Các loại requests:}
\begin{itemize}
    \item \textbf{PLAY:} Gửi khi ở trạng thái READY, bao gồm Session ID
    \item \textbf{PAUSE:} Gửi khi đang PLAYING, tạm dừng streaming
    \item \textbf{TEARDOWN:} Kết thúc phiên và giải phóng tài nguyên
    \item Tất cả requests đều được gửi qua TCP socket với encoding UTF-8
\end{itemize}
...
\subsubsection{Xử lý RTSP Responses}
...
class Server:
    def main(self):
        ...
        # Bind den port va lang nghe ket noi
...
\textbf{Server initialization:}
\begin{itemize}
    \item Tạo TCP socket cho RTSP
    \item Bind socket đến port chỉ định
    \item Listen và chấp nhận kết nối từ client
    \item Tạo ServerWorker thread cho mỗi client
\end{itemize}

\subsubsection{Nhận RTSP Requests}

\paragraph{Hàm recvRtspRequest()}
Thread này chạy liên tục để nhận RTSP requests từ client.

\begin{lstlisting}[language=Python, caption={Receive RTSP Request Thread}]
def recvRtspRequest(self):
    """Nhan RTSP requests tu client"""
...
\textbf{Chức năng:}
\begin{itemize}
    \item Nhận request từ RTSP socket (TCP)
    \item Decode dữ liệu nhận được
    \item Gọi hàm processRtspRequest() để xử lý
\end{itemize}
...
\subsubsection{Khởi tạo RTP Socket (Server Side)}

Server tạo UDP socket để gửi RTP packets đến client.

\begin{lstlisting}[language=Python, caption={Initialize RTP Socket on Server}]
# Trong ham processRtspRequest() - xu ly SETUP request
if requestType == self.SETUP:
    if self.state == self.INIT:
        print("Processing SETUP")
        try:
            # Khoi tao video stream
            self.clientInfo['videoStream'] = VideoStream(filename)
            self.state = self.READY
            
            # Tao UDP socket ngay cho RTP streaming
            self.clientInfo["rtpSocket"] = socket.socket(
                socket.AF_INET, socket.SOCK_DGRAM
            )
        ...
        # Lay RTP port tu SETUP request
        self.clientInfo['rtpPort'] = request[2].split('=')[1]
\end{lstlisting}

\textbf{Chức năng:}
\begin{itemize}
    \item Tạo UDP socket cho việc gửi RTP packets
    \item Socket được tạo ngay sau khi SETUP request thành công
    \item Lưu RTP port của client từ SETUP request (Transport header)
    \item Socket này sẽ được sử dụng trong hàm \texttt{sendRtp()} để gửi video stream
    \item UDP được chọn vì RTP yêu cầu real-time transmission với overhead thấp
\end{itemize}
...
\subsubsection{Nhận RTP Packets (Cơ bản)}

Client nhận RTP packets từ server và hiển thị video frames.

\begin{lstlisting}[language=Python, caption={Listen for RTP Packets - Basic Version}]
def listenRtp(self):
    """Listen for RTP packets."""
    while True:
        try:
            # Nhan du lieu tu UDP socket
            data = self.rtpSocket.recv(20480)
            
            if data:
                # Tao RTP packet object
                rtpPacket = RtpPacket()
                ...
                # Discard late packets (packet loss handling)
...
\textbf{Chức năng nhận RTP packets cơ bản:}
\begin{itemize}
    \item Nhận dữ liệu từ UDP socket (buffer size 20480 bytes)
    \item Decode RTP packet để lấy header và payload
    \item Kiểm tra sequence number để phát hiện packet loss
    \item \textbf{Late packet handling:} Chỉ hiển thị frame nếu sequence number lớn hơn frame hiện tại
    \item Ghi payload vào file cache và hiển thị lên GUI
    \item Dừng listening khi nhận PAUSE/TEARDOWN event
    \item Đóng RTP socket khi teardown hoàn tất
\end{itemize}

\textbf{Xử lý packet loss cơ bản:}
\begin{itemize}
    \item So sánh \texttt{currFrameNbr} với \texttt{self.frameNbr}
    \item Nếu \texttt{currFrameNbr > self.frameNbr}: Packet đến đúng thứ tự hoặc có packet loss → Hiển thị frame
    \item Nếu \texttt{currFrameNbr <= self.frameNbr}: Late packet (packet cũ đến muộn) → Discard
    \item Cách tiếp cận đơn giản: Bỏ qua packet loss, tiếp tục với frame tiếp theo
\end{itemize}

\subsection{Tóm tắt triển khai RTSP/RTP cơ bản}

\subsubsection{Luồng hoạt động tổng thể}

\begin{enumerate}
    \item \textbf{RTSP Handshake (TCP):}
    \begin{itemize}
        \item Client gửi SETUP request → Server trả về Session ID và mở RTP socket
        \item Client gửi PLAY request → Server bắt đầu streaming
        \item PAUSE/TEARDOWN để điều khiển playback
    \end{itemize}
    
    \item \textbf{RTP Streaming (UDP):}
    \begin{itemize}
        \item Server: Đọc frame → Encode RTP packet → Gửi qua UDP
        \item Client: Nhận RTP packet → Decode → Hiển thị frame
        \item Frame rate control: 25 FPS (40ms/frame)
    \end{itemize}
    
    \item \textbf{Session Management:}
    \begin{itemize}
        \item Mỗi client có Session ID riêng biệt
        \item CSeq number để đồng bộ RTSP requests/responses
        \item State machine đảm bảo thứ tự các thao tác
    \end{itemize}
\end{enumerate}

\subsubsection{Các thành phần chính}

\paragraph{RTSP Client}
\begin{itemize}
    \item \textbf{State machine:} INIT → READY → PLAYING
    \item \textbf{4 RTSP methods:} SETUP, PLAY, PAUSE, TEARDOWN
    \item \textbf{TCP socket:} Gửi/nhận RTSP control messages
    \item \textbf{UDP socket:} Nhận RTP data packets
    \item \textbf{Threading:} Async threads cho recvRtspReply() và listenRtp()
    \item \textbf{Packet loss handling:} Discard late packets (currFrameNbr > frameNbr)
\end{itemize}

\paragraph{RTSP Server}
\begin{itemize}
    \item \textbf{Multi-threading:} Mỗi client có worker thread riêng
    \item \textbf{TCP listener:} Nhận và xử lý RTSP requests
    \item \textbf{UDP streaming:} Gửi RTP packets đến client port
    \item \textbf{Session management:} Random Session ID (100000-999999)
    \item \textbf{Video file handling:} VideoStream class đọc frames tuần tự
\end{itemize}

\paragraph{RTP Packet Format}
\begin{itemize}
    \item \textbf{Header:} 12 bytes theo RFC 3550
    \item \textbf{Version:} 2 (RTP v2)
    \item \textbf{Payload Type:} 26 (MJPEG)
    \item \textbf{Sequence Number:} Tăng dần theo frame number
    \item \textbf{Timestamp:} Unix time (seconds)
    \item \textbf{Marker bit:} 0 trong triển khai cơ bản (1 frame = 1 packet)
    \item \textbf{SSRC:} 0 (single source)
\end{itemize}

\subsubsection{Hạn chế của triển khai cơ bản}

\begin{itemize}
    \item \textbf{Frame size limit:} Chỉ hỗ trợ frame < 20480 bytes (buffer size)
    \item \textbf{Không fragmentation:} 1 frame = 1 packet → Không phù hợp HD video
    \item \textbf{Packet loss:} Chỉ discard late packets, không có recovery mechanism
    \item \textbf{No buffering:} Hiển thị frame ngay khi nhận → Có thể bị jitter
    \item \textbf{Fixed frame rate:} 25 FPS cố định, không adaptive
\end{itemize}

\textbf{Để hỗ trợ HD video streaming,} cần triển khai các tính năng nâng cao:
\begin{itemize}
    \item Frame fragmentation (chia frame thành nhiều packets)
    \item Frame buffer (giảm jitter)
    \item Packet reassembly (ghép lại fragments)
    \item Network adaptation (điều chỉnh quality theo bandwidth)
\end{itemize}

→ Xem Section 4: Hỗ trợ HD Video Streaming
