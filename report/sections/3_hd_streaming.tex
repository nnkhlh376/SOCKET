...
Chia mỗi frame thành nhiều RTP packets, mỗi packet $\leq$ MTU:
...
def makeRtp(self, payload, frameNbr, marker=0):
    ...
    extension = 0
    cc = 0
    # marker bit = 1 CHI KHI la fragment cuoi cung
        ...
        try:
            ...
            if frame_size <= CHUNK_SIZE:
                ...
                for i in range(num_fragments):
                    ...
                    # Marker = 1 CHO fragment cuoi cung
...
\begin{enumerate}
    ...
    \item Nếu frame size $\leq$ CHUNK\_SIZE: gửi 1 packet với marker=1
...
\textbf{Metrics:}
\begin{itemize}
    \item \textbf{Bandwidth:} Bytes received $\times$ 8 / elapsed time
...
\begin{itemize}
    \item \textbf{MTU-aware fragmentation:} Chia frames thành packets $\leq$ 1400 bytes
...
\begin{itemize}
    \item \textbf{Server:} Fragment size = min(CHUNK\_SIZE, remaining) $\leq$ 1400 bytes
...
Resolution support & SD only & Full HD (1920$\times$1080) \\
...
\begin{itemize}
    \item [-] Tăng số packets (1 frame $\rightarrow$ 10-150 packets)
